# ISY1101-EP2 — Microservicio de Despachos

Microservicio REST de gestión de **despachos** para **Innovatech Chile**, desarrollado como parte del proyecto semestral ISY1101 EP2. Expone una API que permite crear, consultar y administrar registros de despachos, con persistencia en MySQL 8.
...
git clone https://github.com/serobless/ISY1101-EP2-back-despachos.git
cd ISY1101-EP2-back-despachos
...
- `mysql-despachos` — MySQL 8.0 con healthcheck
- `app-despachos` — Spring Boot, espera a que MySQL esté healthy antes de iniciar
...
|---|---|---|
| `SPRING_DATASOURCE_URL` | URL de conexión JDBC completa | `jdbc:mysql://mysql-despachos:3306/despachos_db?useSSL=false&serverTimezone=UTC&createDatabaseIfNotExist=true&allowPublicKeyRetrieval=true` |
| `DB_USERNAME` | Usuario de la base de datos | `despachos_user` |
| `DB_PASSWORD` | Contraseña de la base de datos | `despachos_pass` |
...
push → deploy branch
  ...
  │ 2. docker pull                       │
  │ 3. docker stop/rm app-despachos      │
  │ 4. docker run -p 8081:8081           │
